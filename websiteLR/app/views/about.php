<!DOCTYPE HTML>

<head>
  <title> Сайт Душниловой Дашки </title>
  <link rel="stylesheet" href="../../../../public/assets/css/about_style.css">
</head>

<body bgcolor="#000000" ;>
  <h1 align="center" ; style="color:#ffffff"> <font face="Book Antiqua"> Сайт Душниловой Дашки </font> </h1>
  <nav>
    <ul>
        <li><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/menu/menu.html">Главная</a></li>
        <li class="on"><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/about/about.html">Обо мне</a></li>
        <li><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/hobby/hobby.html">Мои интересы</a></li>
        <li><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/studies/studies.html">Учёба</a></li>
        <li><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/pics/pics.html">Фотоальбом</a></li>
        <li><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/contact/contact.html">Контакт</a></li>
        <li><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/test/test.html">Тест</a></li>
        <li><a href="file:///C:/Users/Даша/Desktop/Website/ЛР_3/history/history.html">История</a></li>
        <li><a href="">Отзыв</a></li>
        <li><a href="">Блог</a></li>
    </ul>
  </nav>

  <div class="info">
    <h2><img align="left" src="../../../../public/assets/img/MiraAndI.jpg" width="180" height="320"> Я Данилова Дарья Дмитриевна </h2>
    <h4>студентка группы ПИ/б-19-1-о СевГУ</h4>
    <p>Родилась 23.12.2001, что делает меня пессимистичным козерогом.
      <br> Живу Father-Free life и хейчу патриархат.
      <br>Хозяйка и по совместительству главная ФАНАТКА своей пёси мопса Миранды.
      <br> </p>
  </div>

  <div class="container">

    <a href="#" class="link"> Больше моей песи </a>
    <div class="linc2"><img src="../../../../public/assets/img/Mira.jpg"></div>

  </div>

  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
  <script src="../../../../public/assets/js/about.js"></script>
</body>